...
**SPL NAND boot was attempted and reverted.** Enabling `CONFIG_SPL_NAND_SUPPORT=y` + friends builds cleanly but the SPL link fails with undefined references to `nand_spl_load_image`, `nand_spl_adjust_offset`, `nand_init`, `nand_register`, `nand_calculate_ecc`, `nand_correct_data`, `nand_deselect`. Root cause: u-boot-xlnx 2024.01 has no Zynq 7000 SPL NAND loader — `nand_spl_load_image()` ships only for Denali / DaVinci / FSL / MXC / MXS / Sunxi / LPC / MT7621. `drivers/mtd/nand/raw/zynq_nand.c` has only the full-u-boot driver, no SPL variant, and `meta-xilinx` does not add one either (the canonical Xilinx NAND-boot path is FSBL via `meta-xilinx-standalone`, which this layer explicitly opts out of). Two secondary traps along the way: `xilinx_zynq_virt_defconfig` ships `CONFIG_SPL_OS_BOOT=y`, so `SPL_NAND_SUPPORT=y` uncovers a new `hex` prompt `SYS_NAND_SPL_KERNEL_OFFS` with no default → `cml1_do_configure`'s `yes '' | make oldconfig` spins forever on unanswerable prompts and the log grows without bound; a second cascade hits `SYS_NAND_BLOCK_SIZE` the same way. If NAND-boot is ever pursued, either add a Zynq SPL NAND loader or bring back FSBL — see README.md "NAND boot" section.
